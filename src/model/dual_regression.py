...
class DBHNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.rgb_enc = timm.create_model("resnet34", pretrained=True, features_only=True)
        self.depth_enc = timm.create_model("resnet18", pretrained=False, features_only=True)

        self.fuse = nn.Conv2d(512, 256, 3, padding=1)
        self.head = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(256, 1))

    def forward(self, rgb, depth):
        fr = self.rgb_enc(rgb)[-1]
        fd = self.depth_enc(depth)[-1]
        x = torch.cat([fr, fd], dim=1)
        x = self.fuse(x)
        return self.head(x)
